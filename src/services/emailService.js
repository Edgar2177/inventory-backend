...
// ============================================================
// Enviar lista de preparación a cocina (Pre-Batch Ordering)
// ============================================================
const generatePrepEmailHTML = (data) => {
  const { store_name, items } = data;
  const dateStr = new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' });

  const rows = items.map(it => {
    const qty  = (it.actual_order !== undefined && it.actual_order !== null && it.actual_order !== '')
      ? it.actual_order : it.suggested_order;
    const unit = it.yield_unit || '';
    return `
      <tr>
        <td style="padding:10px 12px; border-bottom:1px solid #e2e8f0; color:#1e293b;">${it.prep_name}</td>
        <td style="padding:10px 12px; border-bottom:1px solid #e2e8f0; text-align:right; color:#64748b;">${it.stock_on_hand ?? 0}</td>
        <td style="padding:10px 12px; border-bottom:1px solid #e2e8f0; text-align:right; color:#64748b;">${it.par ?? 0}</td>
        <td style="padding:10px 12px; border-bottom:1px solid #e2e8f0; text-align:right; font-weight:700; color:#0284c7;">${qty} ${unit}</td>
      </tr>`;
  }).join('');
  ...
  <div style="font-family: Arial, sans-serif; max-width:640px; margin:0 auto; color:#1e293b;">
    <div style="background:#0ea5e9; color:#fff; padding:18px 22px; border-radius:10px 10px 0 0;">
      <h2 style="margin:0; font-size:20px;">Preparation List</h2>
      <p style="margin:4px 0 0; opacity:.9; font-size:13px;">${store_name} &middot; ${dateStr}</p>
    </div>
    <div style="border:1px solid #e2e8f0; border-top:none; border-radius:0 0 10px 10px; padding:0;">
      <table style="width:100%; border-collapse:collapse; font-size:14px;">
        <thead>
          <tr style="background:#f8fafc;">
            <th style="padding:10px 12px; text-align:left;  color:#475569; border-bottom:2px solid #e2e8f0;">Preparation</th>
            <th style="padding:10px 12px; text-align:right; color:#475569; border-bottom:2px solid #e2e8f0;">Stock</th>
            <th style="padding:10px 12px; text-align:right; color:#475569; border-bottom:2px solid #e2e8f0;">Par</th>
            <th style="padding:10px 12px; text-align:right; color:#475569; border-bottom:2px solid #e2e8f0;">To Prepare</th>
          </tr>
        </thead>
        <tbody>${rows}</tbody>
      </table>
    </div>
    <p style="color:#94a3b8; font-size:12px; margin-top:14px; text-align:center;">
      Automatically generated by the inventory system.
    </p>
  </div>`;
};

const sendPrepProductionEmail = async (data) => {
  const { to, cc, store_name, items } = data;
  if (!to) throw new Error('A destination email is required');

  const mailOptions = {
    from:    `"${store_name}" <${process.env.EMAIL_USER}>`,
    to:      to,
    cc:      cc || undefined,
    subject: `Preparation List - ${store_name}`,
    html:    generatePrepEmailHTML(data),
  };

  try {
    const info = await transporter.sendMail(mailOptions);
    console.log(`Prep list email sent successfully: ${info.messageId}`);
    return { success: true, messageId: info.messageId };
  } catch (error) {
    console.error('Prep email send error:', error.message);
    throw error;
  }
};
